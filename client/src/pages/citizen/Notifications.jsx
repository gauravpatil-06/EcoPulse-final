...
const Notifications = () => {
    ...
    useEffect(() => {
        fetchNotifications();
        const interval = setInterval(fetchNotifications, 5000); // Poll every 5s for updates
    ...
    const fetchNotifications = async () => {
        try {
            ...
            console.log("Citizen Notifications API Response:", res.data);
    ...
    const getIcon = (type, title) => {
        ...
        if (type === 'Reward' || title.includes('pts') || title.includes('points')) return <span className="text-xl">💰</span>;
        if (title.includes('Cleaned') || title.includes('🎉') || title.includes('Resolved')) return <CheckCircle2 className="text-emerald-500" size={20} />;
        if (title.includes('Request') || title.includes('Accepted')) return <Truck className="text-indigo-500" size={20} />;
            ...
            <div className="min-h-[500px]">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-32 opacity-60">
                        ...
                        <p className="text-[13px] font-black text-slate-400">Loading alerts...</p>
                    </div>
                ) : notifications.length > 0 ? (
            ...
            <AnimatePresence>
                {isClearModalOpen && (
                    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm lg:pl-[260px]">
                        ...
                        >
                            <div className="w-16 h-16 max-sm:w-14 max-sm:h-14 bg-rose-50 dark:bg-rose-500/10 rounded-2xl flex items-center justify-center mx-auto mb-5">
                                <Trash2 size={28} className="text-rose-500" />
                            </div>
                            
                            <h3 className="text-lg font-black text-slate-800 dark:text-white mb-2 tracking-tight">Clear Notifications?</h3>
                            <p className="text-[13px] max-sm:text-[12.5px] font-medium text-slate-500 dark:text-slate-400 leading-relaxed mb-6 px-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button 
                                    ...
                                    className="py-3 max-sm:py-2.5 px-4 bg-slate-50 dark:bg-white/5 hover:bg-slate-100 dark:hover:bg-white/10 text-slate-600 dark:text-slate-300 font-black text-[13px] max-sm:text-[12px] rounded-2xl transition-all active:scale-95"
                                >
                                    No, keep it
                                </button>
                                <button 
                                    onClick={clearAllNotifications}
                                    className="py-3 max-sm:py-2.5 px-4 bg-emerald-500 hover:bg-emerald-600 text-white font-black text-[13px] max-sm:text-[12px] rounded-2xl shadow-lg shadow-emerald-500/20 transition-all active:scale-95"
                                >
                                    Yes, delete
                                </button>
                            </div>
